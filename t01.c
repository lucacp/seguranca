...
int main(int argc,char* args[]){
	...
	switch(tipo){
		...
		case 'a':{
			...
			char chave_subs[CHARACTERES];
			memset(chave_key,0,sizeof(char)*ind_arq);
			memset(chave_subs,0,sizeof(char)*CHARACTERES);
			if(typ=='c'){
		...
		//		chave=comparacaoCifraTransposicao(arq,ind_arq,out);
				...
				int i=0;
				i=comparacaoCifraSubstituicao(arq,ind_arq,out,chave_subs);
				for(i = 0; i< CHARACTERES;i++){
					if(i>0) printf(",");
					printf("%d",chave_subs[i]);
				};
				printf("\n");
			};
			break;
...
void cesarCrypt(FILE *arq,int key,int ind_arq,int arquivo,FILE *out){
	...
	while(ind_arq>ind_atual){
		...
		if(ind_arq<MAX_PRINT)
			printf("%c",c[0]);
		c[0]=(c[0]+(char)key+CHARACTERES)%CHARACTERES;
		if(arquivo==1){
...
int substituicaoCrypt(FILE *arq,char *key,int ind_arq,int arquivo,FILE *out ,int dcrpt ){
	char c[2]=" ",k[3];
	int principal[CHARACTERES],ind_key=0,ind_atual=0,id=0,campo=0;
	/*for( campo = 0, digito=0 ; campo < CHARACTERES ; campo++ ){ if(dcrpt==1){if( campo < id_key){bytes[campo]=campo;bytes[(int)key[campo]]=key[campo];}else if(bytes[campo]==' '){if(bytes[digito]==' '){bytes[campo]=campo;bytes[digito]=digito;digito++;}else{while(digito<CHARACTERES){++digito;if(bytes[digito]==' '){bytes[campo]=campo;bytes[digito]=digito;digito++;break;}};};}}else if( campo < id_key){bytes[campo]=key[campo];bytes[(int)key[campo]]=campo;}else if(bytes[campo]==' '){if(bytes[digito]==' '){bytes[campo]=digito;bytes[digito]=campo;digito++;}else{while(digito<CHARACTERES){++digito;if(bytes[digito]==' '){bytes[campo]=digito;bytes[digito]=campo;digito++;break;}};};};};if(dcrpt==1){for(campo=0;campo<CHARACTERES;campo++){bytes[campo]*=(-1);};};*/
	FILE *chave_arq=NULL;
	...
	while(ind_arq>ind_atual){
		...
		c[0]=fgetc(arq);
		//printf("%d",c[0]);
		campo=(c[0]+CHARACTERES)%CHARACTERES;
		c[0]=principal[campo];
		if(arquivo==1){
			fwrite(c,sizeof(char),1,out);
			fflush(out);
...
int ComparacaoCifraCesar(FILE *arq1,int ind_arq,FILE *arq2){
	...
	while(ind_arq>ind_atual){	
		...
		fflush(arq1);
		fflush(arq2);
		chave_atual=(c2-c1+CHARACTERES)%CHARACTERES;
...
int comparacaoCifraSubstituicao(FILE *arq1,int ind_arq,FILE *arq2,char *chave){
	/* indice é o caracter criptografado arq2
	 * conteudo é o caracter decriptografado arq1
	 * há chance de não utilizar todos os possiveis caracteres **/
	char c1=' ',c2=' ';
	int ind_atual=0,ind_chav=0,campo=0;
	while(ind_atual<ind_arq){
		fread(&c1,sizeof(char),1,arq1);
		fread(&c2,sizeof(char),1,arq2);
		fflush(arq1);
		fflush(arq2);
		campo=(c2+CHARACTERES)%CHARACTERES;
		if(chave[campo]==0){
			chave[campo]=(c1+CHARACTERES)%CHARACTERES;
			ind_chav++;
		}
		ind_atual++;
	};
...
int comparacaoCifraTransposicao(FILE *arq1,int ind_arq1,FILE *arq2){
	/* Para cifra de transição é necessário achar os primeiros characteres "base" para achar a chave
	 * assim quando for visto que a diferença atual aumenta, quer dizer que ainda não achou o character certo
	 * com excesão da primeira vez que calcula a distancia.
